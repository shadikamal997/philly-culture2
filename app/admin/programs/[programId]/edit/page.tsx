  ...
  useEffect(() => {
    const fetchProgramData = async () => {
      ...
      try {
        ...
        // Access control - convert from backend format to simplified UI
        const unlockType = program.unlockType || "instant";
        const dripInterval = program.dripInterval || 1;
        if (unlockType === "instant") {
          setDripSchedule("none");
        } else if (dripInterval === 1) {
          setDripSchedule("daily");
        } else if (dripInterval === 7) {
          setDripSchedule("weekly");
        } else if (dripInterval === 30) {
          setDripSchedule("monthly");
        } else {
          setDripSchedule("none"); // fallback
        }
